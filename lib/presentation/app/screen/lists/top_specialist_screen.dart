...
class _TopSpecialistScreenState extends State<TopSpecialistScreen> {
  ...
  Expanded buildSpecialistListView(BuildContext context) {
    ...
    double width = (MediaQuery.of(context).size.width - (margin * crossCount)) /
        crossCount;
    double height = 208.h;
    List<Specialist> topSpecialist = homeController.homeData.value!.specialist!;
    ...
    return Expanded(
      ...
      children: List.generate(topSpecialist.length, (index) {
        Specialist specialist = topSpecialist[index];
        return InkWell(
          onTap: () {
            Constant.sendToNext(context, Routes.specialistDetailScreenRoute, arguments: specialist.id);
          },
          child: getShadowDefaultContainer(
            margin: EdgeInsets.only(
                left: (index.isEven) ? margin : 0.h,
                right: (index.isOdd) ? margin : 0),
            height: height,
            color: Colors.white,
            child: Column(
              children: [
                Expanded(
                    child: getCircleNetworkImage(
                        context, specialist.image!, double.infinity)),
                getVerSpace(16.h),
                getCustomFont(specialist.name!, 16.sp, Colors.black, 1,
                    fontWeight: FontWeight.w700, txtHeight: 1.4.h),
                getVerSpace(4.h),
                getCustomFont(specialist.education!, 15.sp, greyFontColor, 1,
